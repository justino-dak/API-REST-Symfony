 ...
 * @Hateoas\Relation(
 * "self",
 * href=@Hateoas\Route("detailAuthor", parameters={"id"="expr(object.getId())"}),
 * exclusion=@Hateoas\Exclusion(groups="getAuthors", excludeIf="expr(not is_granted('ROLE_ADMIN'))"),
 * embedded="expr(object.getBooks())"
 * )
 * 
 * @Hateoas\Relation(
 * "delete",
 * href=@Hateoas\Route("deleteAuthor",parameters={"id"="expr(object.getId())" }),
 * exclusion=@hateoas\Exclusion(groups="getAuthors",excludeIf="expr(not is_granted('ROLE_ADMIN'))")
 * )
 * @Hateoas\Relation(
 * "update",
 * href=@Hateoas\Route("updateAuthor",parameters={"id"="expr(object.getId())" }),
 * exclusion=@hateoas\Exclusion(groups="getAuthors",excludeIf="expr(not is_granted('ROLE_ADMIN'))")
 * )
 * 
 */
...
class Author
{
    ...
    #[Assert\Length(min: 1 ,max: 255, minMessage: 'Le prénom doit faire au moins  {{ limit }} caractères', 
        maxMessage: 'Le prénom de l\'auteur ne peut pas plus de {{ limit }} caractères')]
    ...
    #[Assert\Length(min: 1 ,max: 255, minMessage: 'Le nom doit faire au moins {{ limit }} caractères', 
        maxMessage: 'Le nom de l\'auteur ne peut pas plus de {{ limit }} caractères')]
